...
//添加连接属性
func (self *Connection) AddProperty(key string, val interface{}) {
	self.PropertiesLock.Lock()
	defer self.PropertiesLock.Unlock()
...
func (self *Connection) GetProperty(key string) (interface{}, error) {
	self.PropertiesLock.RLock()
	defer self.PropertiesLock.RUnlock()
	if val, ok := self.Properties[key]; ok {
		return val, nil
	} else {
